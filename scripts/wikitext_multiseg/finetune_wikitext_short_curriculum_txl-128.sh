...
horovodrun --gloo -np $NP python run_finetuning_lm_multiseg_rmt_chunked.py \
        ...
        --model_cls $MODEL_CLS \
        --model_cpt ../runs/lm_long/${TASK_NAME}/$MODEL_NAME/${SCHEDULER}_adamw_wd1e-03_$(((INPUT_SIZE-2*MEMORY_SIZE)*(MAX_N_SEGMENTS-1)))-${TGT_LEN}-$((MAX_N_SEGMENTS-1))x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-$((K2-1))_from_cpt_$((MAX_N_SEGMENTS-2))-$((MAX_N_SEGMENTS-1))_segm_valid/run_$N \
