...
horovodrun --gloo -np $NP python run_finetuning_lm_multiseg_rmt_chunked.py \
        ...
        --model_cls $MODEL_CLS \
        --model_cpt ../runs/lm_long/${TASK_NAME}/gpt2/linear_adamw_wd1e-03_$(((INPUT_SIZE-2*MEMORY_SIZE)*SOURCE_N_SEGMENTS))-128-${SOURCE_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs32_${SEGMENT_ORDERING}_bptt-${K2}_from_cpt_$((SOURCE_N_SEGMENTS-1))-${SOURCE_N_SEGMENTS}_segm_valid/run_$N \
