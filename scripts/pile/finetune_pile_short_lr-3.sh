...
accelerate launch --config_file ./accel_configs/deepspeed_fp16_o2_np1.yaml --main_process_port 29523 run_finetuning_pile_rmt_deepspeed.py \
        --task_name $TASK_NAME \
...
echo RUNNING: $TASK_NAME $MEMORY_SIZE $INPUT_SIZE $BLOCK_SIZE $HISTORY_SIZE $MAX_N_SEGMENTS $MODEL_NAME $MODEL_CLS  $LR $N
echo gradient accumulation steps $(($TBS/($BS*$NP)))
accelerate launch --config_file ./accel_configs/fp16-1.yaml --main_process_port 29523 run_finetuning_pile_rmt.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${BLOCK_SIZE}-${HISTORY_SIZE}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}/run_$N \
        --from_pretrained $MODEL_NAME \
        --model_type $MODEL_TYPE \
