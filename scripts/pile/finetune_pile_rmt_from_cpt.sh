...
MAX_N_SEGMENTSS=(1 2)
BSS=(16 8)
ACCEL_CONFIGS=(./accel_configs/deepspeed_fp16_o2_np4-4.yaml ./accel_configs/deepspeed_fp16_o2_np4-8.yaml)
...
accelerate launch --config_file ${ACCEL_CONFIG} --main_process_port 29511 run_finetuning_pile_rmt_deepspeed.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${HISTORY_SIZE}-${BLOCK_SIZE}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_from_cpt_0-${MAX_N_SEGMENTS}/run_$N \
