...
horovodrun --gloo -np $NP python run_finetuning_arxiv_noise_rmt.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${TGT_LEN}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_from_cpt_$((MAX_N_SEGMENTS-1))-${MAX_N_SEGMENTS}_noise-${NOISE_N_SEGMENTS}/run_$N \
