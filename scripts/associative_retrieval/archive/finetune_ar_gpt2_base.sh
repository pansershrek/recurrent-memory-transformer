...
MODEL_CFG=/home/jovyan/rmt/wip/base_models/gptconfigs/gpt2_tiny.json
...
BLOCK_SIZE=$(((KEY_SIZE + VALUE_SIZE + 2)*2))
...
echo RUNNING: TASK_NAME MEMORY_SIZE KEY_SIZE VALUE_SIZE BLOCK_SIZE N_SEG  MODEL_NAME MODEL_CLS LR N
echo RUNNING: $TASK_NAME $MEMORY_SIZE $KEY_SIZE $VALUE_SIZE $BLOCK_SIZE $MAX_N_SEGMENTS $MODEL_NAME $MODEL_CLS  $LR $N
accelerate launch --num_processes $NP --config_file ./accelerate.yaml --main_process_port 29500 run_finetuning_associative_retrieval.py \
        --task_name $TASK_NAME \
