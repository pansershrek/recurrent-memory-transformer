...
accelerate launch --config_file ./accel_configs/fp16-1.yaml --main_process_port 29525 run_finetuning_associative_retrieval.py \
        --task_name $TASK_NAME \
        --model_path ../runs/test/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_k${KEY_SIZE}-v${VALUE_SIZE}-p${NUM_PAIRS}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}/run_$N \
        --model_cfg $MODEL_CFG \
        ...
        --data_n_workers 2 \
        --log_interval $(($ITERS/10)) --valid_interval $(($ITERS/2)) \
