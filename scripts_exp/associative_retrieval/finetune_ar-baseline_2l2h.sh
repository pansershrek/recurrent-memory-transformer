...
for VALUE_SIZE in 2 4 8
do

BLOCK_SIZE=$(((KEY_SIZE + VALUE_SIZE + 2) * NUM_PAIRS * 2))
...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29572 run_finetuning_associative_retrieval.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_k${KEY_SIZE}-v${VALUE_SIZE}-p${NUM_PAIRS}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}+2l2h/run_$N \
        --model_cfg $MODEL_CFG \
