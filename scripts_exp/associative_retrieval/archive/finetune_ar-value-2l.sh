...
MODEL_CFG=/home/jovyan/rmt/wip/base_models/gptconfigs/neox_tiny_2l2h.json
python create_config.py --hidden_size 512 --num_hidden_layers 2 --num_attention_heads

KEY_SIZE=4
NUM_PAIRS=1

for VALUE_SIZE in 1 2 4 8 16 32 64 128 256 512
do
...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29571 run_finetuning_associative_retrieval.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_k${KEY_SIZE}-v${VALUE_SIZE}-p${NUM_PAIRS}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_2l2h/run_$N \
        --model_cfg $MODEL_CFG \
