...
for VALUE_SIZE in 8 16 32 64 128
do
...
accelerate launch --num_processes $NP --config_file ./accelerate.yaml --main_process_port 29503 run_finetuning_associative_retrieval.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_k${KEY_SIZE}-v${VALUE_SIZE}-p${NUM_PAIRS}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_from_cpt/run_$N \
        --model_cfg $MODEL_CFG \
        --model_cls $BACKBONE_CLS \
        --model_cpt ../runs/associative_retrieval/lr1e-03_linear_adamw_wd1e-03_k4-v16-p1-1x2048_mem4_bs512_regular_bptt-1/run_4/\
