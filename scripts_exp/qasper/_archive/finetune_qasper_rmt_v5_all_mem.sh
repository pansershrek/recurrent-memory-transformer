...
MODEL_TYPE=decoder
MEMORY_CELL=modeling_rmt.experimental:MemoryCellAllMemories
RECURRENT_WRAPPER=modeling_rmt.experimental:RecurrentWrapperAllMemories
...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29586 run_finetuning_scrolls_rmt_decoder_v5.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_iters${ITERS}_${SEGMENT_ORDERING}_v5_all_mem/run_$N \
