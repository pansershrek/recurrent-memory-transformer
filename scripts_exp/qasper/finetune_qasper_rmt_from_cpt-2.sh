...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29572 run_finetuning_scrolls_rmt_decoder.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_iters${ITERS}_${SEGMENT_ORDERING}_v2_from_cpt_0-${MAX_N_SEGMENTS}/run_$N \
        --from_pretrained $MODEL_NAME \
        --model_type $MODEL_TYPE \
        --memory_cell_cls $MEMORY_CELL \
        --recurrent_wrapper_cls $RECURRENT_WRAPPER \
        --model_cls $BACKBONE_CLS \
        --backbone_cpt ../runs/qasper/EleutherAI/pythia-70m-deduped/lr3e-04_linear_adamw_wd1e-03_1024-1x1024_mem0_bs4_iters1000000_regular_v2/run_1/ \
