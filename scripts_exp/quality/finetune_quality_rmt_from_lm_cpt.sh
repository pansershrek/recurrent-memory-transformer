...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29597 run_finetuning_scrolls_rmt_decoder_v3.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${INPUT_SEQ_LEN}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_iters${ITERS}_${SEGMENT_ORDERING}_v3_from_lm_cpt_2-${MAX_N_SEGMENTS}/run_$N \
        --from_pretrained $MODEL_NAME \
        --model_type $MODEL_TYPE \
        --memory_cell_cls $MEMORY_CELL \
        --recurrent_wrapper_cls $RECURRENT_WRAPPER \
        --model_cls $BACKBONE_CLS \
        --model_cpt ../runs/pile/EleutherAI/pythia-70m-deduped/lr1e-05_linear_adamw_wd1e-03_2044-2044-2x2048_mem2_bs256_regular_bptt-2_from_cpt_cv4_sp0.2_1-2/run_1 \
