...
SEGMENT_SIZE=1024 # size of one segment in tokens
SAMPLE_SIZE=1024 # length of task sample in tokens
MEMORY_SIZE=0
MAX_N_SEGMENTS=1

TASK_START=0.2 # facts start at >20% of length
TASK_END=0.4   # facts end at <40% of sample length
MAX_N_FACTS=10 # limit number of facts per sample for precision
...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29002 run_finetuning_babilong_rmt.py \
        ...
        --model_path /home/bulatov/runs/babilong/${TASK_DATASET}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_tp${TASK_START}-${TASK_END}/run_$N \
        ...
        --task_start_pct $TASK_START \
        --task_end_pct $TASK_END \
