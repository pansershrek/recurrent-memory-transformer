...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29515 run_finetuning_pile_rmt.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/${SCHEDULER}_adamw_wd1e-03_${BLOCK_SIZE}-${EVAL_HISTORY_SIZE}-${EVAL_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_from_cpt_cv4_eval_32k+_20kdoc_${MAX_N_SEGMENTS}-${EVAL_N_SEGMENTS}/run_$N \
        --from_pretrained $MODEL_NAME \
        --model_cpt ../runs/${TASK_NAME}/$MODEL_NAME/linear_adamw_wd1e-03_2044-10220-6x2048_mem2_bs256_regular_bptt-6_from_cpt_cv4_sp1_5-6/run_2/model_30000/ \
