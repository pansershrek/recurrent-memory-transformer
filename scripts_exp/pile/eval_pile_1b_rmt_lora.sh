...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29566 run_finetuning_pile_rmt.py \
        --task_name $TASK_NAME \
        --model_path ../runs/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd1e-03_${BLOCK_SIZE}-${EVAL_HISTORY_SIZE}-${EVAL_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_sp0.2_lora_eval_32k+_20kdoc_${MAX_N_SEGMENTS}-${EVAL_N_SEGMENTS}/run_$N \
        --from_pretrained $MODEL_NAME \
        --backbone_cpt ../runs/${TASK_NAME}/EleutherAI/pythia-1b-deduped/lr1e-04_linear_adamw_wd1e-03_0-2048-1x2048_mem0_bs256_regular_bptt-1_sp0.2_lora/run_1/ \
