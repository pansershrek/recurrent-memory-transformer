...
parser.add_argument('--segment_ordering', type=str, help='segment order', default='regular',
                    choices=['regular', 'reverse', 'bidirectional', 'repeat_first', 'last_memory_only'])
...
if __name__ == '__main__':
    ...
    if args.model_type == 'decoder':
        ...
        def get_sample(q, c, a):
            ...
            answer = a_tok + a + [eos_token]
            last_seg = copy.deepcopy(segments[-1])
            last_seg[-len(answer):] = answer
            segments[-1] = last_seg

            full_input = [t for s in segments for t in s]
            gen_input = full_input[:-len(a) - 1]
        ...
        def collate_fn(batch):
            ...
            labels = tokenizer.batch_encode_plus(target_text, truncation=False, padding=False)['input_ids']
